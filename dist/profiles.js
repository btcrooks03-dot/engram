...
export function listProfiles(memoryDir) {
...
export function saveProfile(memoryDir, profileName) {
...
export function loadProfile(memoryDir, profileName) {
    const safeName = sanitizeProfileName(profileName);
    if (!safeName)
...
export function deleteProfile(memoryDir, profileName) {
    const safeName = sanitizeProfileName(profileName);
    if (!safeName)
        return "Invalid profile name.";
    const projectKey = getProjectKey(memoryDir);
...
export function diffProfile(memoryDir, profileName) {
    const safeName = sanitizeProfileName(profileName);
    const projectKey = getProjectKey(memoryDir);
    const profileDir = path.join(PROFILES_DIR, projectKey, safeName);
