...
import { z } from "zod";
import { DATA_DIR, STOP_WORDS } from "./constants.js";
import { readJson, writeJson, ensureDir, parseFrontmatter } from "./helpers.js";
...
export function loadTeamConfig() {
    return readJson(TEAM_CONFIG_PATH);
}
...
function calculateEffectivenessForFiles(files) {
...
function scanSharedMemoryDir(sharedDir) {
...
// ─── Tool Registration ──────────────────────────────────────────────────────
export function registerSharedTools(server) {
    server.tool("engram_team_config", "Get or set team configuration for shared memory. When called with no arguments, returns current config. When called with arguments, updates the config. Shared memory lets teams maintain a common set of memory files that all members can access.", {
    ...
    server.tool("engram_scan_shared", "Scan the shared memory directory and return files with effectiveness scores. Also provides a combined view showing personal + shared memories together. Requires team config to be set up first (see engram_team_config).", {
        memory_dir: z
            .string()
            .optional()
    ...
    server.tool("engram_team_health", "Analyze team memory health. Reports on shared memory quality, contributor activity (if git repo), common gaps across the team, and type coverage. Helps identify what shared knowledge is missing or needs improvement.", {}, async () => {
        const config = loadTeamConfig();
        if (!config) {
            return {
                content: [
        ...
        if (lowEffectiveness.length > 0) {
            gaps.push(`${lowEffectiveness.length} shared memory file(s) score below 50/100 effectiveness — review for quality`);
        }
        ...
        if (staleFiles.length > 0) {
            gaps.push(`${staleFiles.length} shared file(s) not updated in 90+ days: ${staleFiles.map((f) => f.filename).join(", ")}`);
        }
        ...
        if (gitRepo) {
            ...
            for (const name of gitContributors) {
                ...
                try {
                    const authorFiles = execSync(`git log --author="${name}" --name-only --pretty=format: --all -- "*.md"`, {
                    ...
                    let lastUpdated = "";
                    try {
                        lastUpdated = execSync(`git log --author="${name}" -1 --format="%aI" --all`, {
                            cwd: config.shared_memory_dir,
                            encoding: "utf-8",
                            timeout: 5000,
                            stdio: ["pipe", "pipe", "pipe"],
        ...
        const avgEffectiveness = sharedEffectiveness.length > 0
            ? Math.round(sharedEffectiveness.reduce((sum, e) => sum + e.score, 0) /
                sharedEffectiveness.length)
            : 0;
        const coverageScore = Math.round(((allTypes.length - missingTypes.length) / allTypes.length) * 100);
        const overallHealth = sharedFiles.length === 0
            ? "empty"
