...
import type { ProjectMemory, Suggestion, DescriptionSuggestion, MergeResult, BootstrapSuggestion } from "./types.js";

export function generateSuggestions(project: ProjectMemory, projectDir?: string): Suggestion[] {
...
export function extractKeyPhrases(text: string): Array<{ phrase: string; score: number }> {
  const words = text
    .toLowerCase()
    .replace(/[^a-z0-9\s-]/g, " ")
    .split(/\s+/)
...
export const DESC_TEMPLATES: Record<string, (name: string, phrases: string[]) => string> = {
  user: (name, phrases) =>
...
export function generateDescriptions(memoryDir: string): DescriptionSuggestion[] {
...
export function generateMerge(memoryDir: string, file1: string, file2: string): MergeResult | string {
...
export function bootstrapScan(projectDir: string): {
