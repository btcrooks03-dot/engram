...
import { readJson, ensureDir } from "./helpers.js";

// ─── Types ──────────────────────────────────────────────────────────────────

export interface MemoryTemplateEntry {
  filename: string;
...
export interface MemoryTemplate {
  ...
  name: string;
  description: string;
  category: "role" | "domain" | "workflow";
  memories: MemoryTemplateEntry[];
}

export interface AppliedMemoryFile {
  filename: string;
  type: string;
  name: string;
  description: string;
...
const BUILT_IN_DIR = path.resolve(
...
function loadTemplatesFromDir(dir: string): MemoryTemplate[] {
  ...
  try {
    ...
    for (const file of files) {
      ...
      if (data && data.id && data.name && Array.isArray(data.memories)) {
        templates.push(data as MemoryTemplate);
      }
    }
  } catch {
...
export function listAllTemplates(): Array<MemoryTemplate & { source: "built-in" | "custom" }> {
...
export function findTemplate(id: string): (MemoryTemplate & { source: "built-in" | "custom" }) | null {
...
export function registerTemplateTools(server: McpServer): void {
  server.tool(
    "engram_list_templates",
    "List all available memory templates (built-in and custom). Templates provide pre-structured memory file layouts for specific roles, domains, or workflows. Use this to help users bootstrap their memory quickly.",
    {},
    async () => {
  ...
  server.tool(
    "engram_apply_template",
    "Apply a memory template to a memory directory. Returns the file structures ready to create — does NOT auto-create files. Present the results to the user and let them confirm before creating. Includes content_prompts (questions to ask the user) and example_content for each file.",
    {
            ...
            text: JSON.stringify(
              {
                ...
                instructions:
                  "Present these files to the user. For each file, ask the content_prompts to gather information, then create the file with the frontmatter + user's answers. Create MEMORY.md with the suggestedIndex content. Do NOT auto-create without user confirmation.",
              },
              null,
              2
            ),
