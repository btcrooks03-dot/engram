...
function detectChanges(memoryDir: string): FileChange[] {
  const snapshots: Record<string, { size: number; mtime: number }> = readJson(SNAPSHOTS_PATH) || {};
  const newSnapshots: Record<string, { size: number; mtime: number }> = {};
  ...
  async () => {
    ...
    for (let i = 0; i < allFiles.length; i++) {
      for (let j = i + 1; j < allFiles.length; j++) {
        if (allFiles[i].project === allFiles[j].project) continue;
